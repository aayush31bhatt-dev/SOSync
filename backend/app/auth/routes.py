...
async def upload_evidence_file(
    ...
    if len(raw_bytes) > MAX_EVIDENCE_UPLOAD_BYTES:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail="Evidence file is too large.",
        )

    suffix = Path(file.filename or "evidence.bin").suffix.strip()
    if not suffix:
        suffix = ".bin"
    storage_name = f"{uuid.uuid4().hex}{suffix}"
    target_path = EVIDENCE_STORAGE_DIR / storage_name

    target_path.write_bytes(raw_bytes)

    public_base = os.getenv("PUBLIC_BASE_URL", "").strip().rstrip("/")
    if public_base:
        evidence_url = f"{public_base}/evidence/files/{storage_name}"
    else:
        evidence_url = f"{str(request.base_url).rstrip('/')}/evidence/files/{storage_name}"

    return {
        "status": "uploaded",
        "uploaded_by_user_id": current["user"]["user_id"],
...
@router.get("/evidence/files/{storage_name}")
def get_uploaded_evidence_file(storage_name: str):
    safe_name = Path(storage_name).name
    if safe_name != storage_name:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid evidence file name.")

    target_path = EVIDENCE_STORAGE_DIR / safe_name
